...
export function EnvelopeMiniWidget({ tokens, scheme, ...data }: WidgetData & { tokens: ThemeTokens; scheme: 'light' | 'dark' }) {
  ...
  const trendColor = flat ? null : data.weeklyTrend!.dir === 'down' ? tokens.mint : tokens.coral
  const trendDir = !data.weeklyTrend || data.weeklyTrend.dir === 'flat' ? 'up' : data.weeklyTrend!.dir
  return (
    <WidgetSurface tokens={tokens} scheme={scheme} style={{ paddingHorizontal: 10, paddingVertical: 6 }}>
      <FlexWidget style={{ width: 'match_parent', flex: 1, flexDirection: 'column', justifyContent: 'center' }}>
        <TextWidget
          text={data.totalLeft}
          truncate="END"
          maxLines={1}
          style={{ width: 'match_parent', fontSize: 22, fontFamily: fontFamily.displayBold, color: color(tokens.text) }}
        />
        <FlexWidget style={{ width: 'match_parent', flexDirection: 'row', alignItems: 'center' }}>
          ...
          {!flat && trendColor && (
            <FlexWidget style={{ flexDirection: 'row', alignItems: 'center', marginLeft: 8 }}>
              <SvgWidget svg={trendingSvg(trendDir, trendColor)} style={{ width: 10, height: 10, marginRight: 3 }} />
              <TextWidget text={`${data.weeklyTrend!.pct}%`} style={{ fontSize: 10, fontFamily: fontFamily.bodySemiBold, color: color(trendColor) }} />
            </FlexWidget>
          )}
        </FlexWidget>
      </FlexWidget>
